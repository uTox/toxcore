...
int pack_nodes(uint8_t *data, uint16_t length, const Node_format *nodes, uint16_t number)
{
    ...
    for (i = 0; i < number; ++i) {
        ...
        if (ipv6 == 0) {
            ...
            memcpy(data + packed_length + 1 + SIZE_IP4 + sizeof(uint16_t), nodes[i].public_key, CRYPTO_PUBLIC_KEY_SIZE);
            packed_length += size;
        } else if (ipv6 == 1) {
            ...
            memcpy(data + packed_length + 1 + SIZE_IP6 + sizeof(uint16_t), nodes[i].public_key, CRYPTO_PUBLIC_KEY_SIZE);
            packed_length += size;
        } else {
            return -1;
        }
                 ...
                 uint16_t length, uint8_t tcp_enabled)
{
    ...
    while (num < max_num_nodes && len_processed < length) {
        ...
        if (ipv6 == 0) {
            ...
            memcpy(nodes[num].public_key, data + len_processed + 1 + SIZE_IP4 + sizeof(uint16_t), CRYPTO_PUBLIC_KEY_SIZE);
            len_processed += size;
            ++num;
        } else if (ipv6 == 1) {
            uint32_t size = PACKED_NODE_SIZE_IP6;
            ...
            memcpy(nodes[num].public_key, data + len_processed + 1 + SIZE_IP6 + sizeof(uint16_t), CRYPTO_PUBLIC_KEY_SIZE);
            len_processed += size;
            ++num;
        } else {
            return -1;
...
{
    if (index >= CRYPTO_PUBLIC_KEY_SIZE * 8)
        return -1;
...
static int alloc_buckets(DHT_Bucket *bucket)
{
    ...
    if (b0 && b1) {
        ...
        if (bucket->public_key) {
            ...
            if (bit == 0) {
                memcpy(b0->searched_public_key, bucket->searched_public_key, CRYPTO_PUBLIC_KEY_SIZE);
                b0->public_key = 1;
            } else if (bit == 1) {
                memcpy(b1->searched_public_key, bucket->searched_public_key, CRYPTO_PUBLIC_KEY_SIZE);
                b1->public_key = 1;
            }
        }
...
static int recursive_DHT_bucket_add_key(DHT_Bucket *bucket, const uint8_t *public_key)
{
    ...
    if (bucket->public_key) {
        ...
        memcpy(bucket->searched_public_key, public_key, CRYPTO_PUBLIC_KEY_SIZE);
        bucket->public_key = 1;
        return 0;
    }
}
...
static int dealloc_buckets(DHT_Bucket *bucket)
{
    ...
    uint8_t pk[CRYPTO_PUBLIC_KEY_SIZE] = {0};
        ...
        for (i = 0; i < dht->num_friends; ++i) {
            ...
            if (!client_in_nodelist(friend->to_bootstrap, friend->num_to_bootstrap, public_key)) {
                if (friend->num_to_bootstrap < MAX_SENT_NODES) {
                    memcpy(friend->to_bootstrap[friend->num_to_bootstrap].public_key, public_key, CRYPTO_PUBLIC_KEY_SIZE);
                    ...
                    const Node_format *sendback_node)
{
    ...
    uint8_t encrypt[sizeof(plain) + CRYPTO_MAC_SIZE];
    uint8_t data[1 + CRYPTO_PUBLIC_KEY_SIZE + CRYPTO_NONCE_SIZE + sizeof(encrypt)];

    memcpy(plain, client_id, CRYPTO_PUBLIC_KEY_SIZE);
    memcpy(plain + CRYPTO_PUBLIC_KEY_SIZE, &ping_id, sizeof(ping_id));

    uint8_t shared_key[CRYPTO_PUBLIC_KEY_SIZE];
    ...
    memcpy(data + 1, dht->self_public_key, CRYPTO_PUBLIC_KEY_SIZE);
    memcpy(data + 1 + CRYPTO_PUBLIC_KEY_SIZE, nonce, CRYPTO_NONCE_SIZE);
    memcpy(data + 1 + CRYPTO_PUBLIC_KEY_SIZE + CRYPTO_NONCE_SIZE, encrypt, len);
                          ...
                          const uint8_t *sendback_data, uint16_t length, const uint8_t *shared_encryption_key)
{
    ...
    uint8_t data[1 + CRYPTO_PUBLIC_KEY_SIZE + CRYPTO_NONCE_SIZE
                 + Node_format_size * MAX_SENT_NODES + length + CRYPTO_MAC_SIZE];
    ...
    if (len != 1 + nodes_length + length + CRYPTO_MAC_SIZE)
        return -1;
                  ...
                  void *data, int32_t number, uint16_t *lock_count)
{
    ...
    memcpy(friend->public_key, public_key, CRYPTO_PUBLIC_KEY_SIZE);
...
DHT *new_DHT(Logger *l, Networking_Core *net, bool holepunching_enabled)
{
    /* init time */
    unix_time_update();
