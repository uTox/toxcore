...
{   
    ...
    if (device_num >= tox->mdev->device_count) {
        SET_ERROR_PARAMETER(error, TOX_ERR_DEVICE_GET_NODEV);
        return 0;
    }

    memcpy(public_key, tox->mdev->device[device_num].real_pk, sizeof(tox->mdev->device[device_num].real_pk));
