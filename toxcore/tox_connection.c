...
int toxconn_add_tcp_relay(Tox_Connections *tox_conns, int toxconn_id, IP_Port ip_port, const uint8_t *public_key)
{
    ...
    memcpy(tox_con->tcp_relays[index].public_key, public_key, CRYPTO_PUBLIC_KEY_SIZE);
...
static void change_dht_pk(Tox_Connections *tox_conns, int toxconn_id, const uint8_t *dht_public_key)
{
    ...
    memcpy(tox_con->dht_temp_pk, dht_public_key, CRYPTO_PUBLIC_KEY_SIZE);
}
...
static int handle_packet(void *object, int number, const uint8_t *data, uint16_t length, void *userdata)
{
    ...
    if (data[0] == PACKET_ID_FRIEND_REQUESTS) {
        if (tox_conns->friend_request_callback) {
            tox_conns->friend_request_callback(tox_conns->friend_request_object, tox_con->real_public_key, data, length, userdata);
        }
...
int toxconn_get_public_keys(uint8_t *real_pk, uint8_t *dht_temp_pk, Tox_Connections *tox_conns, int toxconn_id)
{
    ...
    if (real_pk)
        memcpy(real_pk, tox_con->real_public_key, CRYPTO_PUBLIC_KEY_SIZE);

    if (dht_temp_pk)
        memcpy(dht_temp_pk, tox_con->dht_temp_pk, CRYPTO_PUBLIC_KEY_SIZE);

    return 0;
}

/* Set temp dht key for connection. */
void set_dht_temp_pk(Tox_Connections *tox_conns, int toxconn_id, const uint8_t *dht_temp_pk, void *userdata)
{
    dht_pk_callback(tox_conns, toxconn_id, dht_temp_pk, userdata);
}
...
int toxconn_set_callbacks(Tox_Connections *tox_conns, int toxconn_id, unsigned int index,
        int (*status_callback)(void *object, int friend_num, int device_id, uint8_t status, void *userdata),
        int (*data_callback)(void *object, int friend_num, int device_id, const uint8_t *data, uint16_t length, void *userdata),
        int (*lossy_data_callback)(void *object, int friend_num, int device_id, const uint8_t *data, uint16_t length, void *userdata),
...
int new_tox_conn(Tox_Connections *tox_conns, const uint8_t *real_public_key)
{
    ...
    tox_con->crypt_connection_id = -1;
    tox_con->status = TOXCONN_STATUS_CONNECTING;
    memcpy(tox_con->real_public_key, real_public_key, CRYPTO_PUBLIC_KEY_SIZE);
...
void set_tox_conn_request_callback(Tox_Connections *tox_conns,
                                   int (*friend_request_callback)(void *, const uint8_t *, const uint8_t *, uint16_t, void *),
                                   void *object)
{
    tox_conns->friend_request_callback = friend_request_callback;
    tox_conns->friend_request_object = object;
    oniondata_registerhandler(tox_conns->onion_c, CRYPTO_PACKET_FRIEND_REQ, friend_request_callback, object);
}
 ...
 */
int send_friend_request_pkt(Tox_Connections *tox_conns, int toxconn_id, uint32_t nospam_num, const uint8_t *data,
                               uint16_t length)
{
        ...
        if (tox_con) {
            if (tox_con->status == TOXCONN_STATUS_CONNECTING) {
                ...
                if (tox_con->ping_lastrecv + TOXCONN_TIMEOUT < temp_time) {
                    ...
                    crypto_kill(tox_conns->net_crypto, tox_con->crypt_connection_id);
                    tox_con->crypt_connection_id = -1;
                    handle_status(tox_conns, i, 0, userdata); /* Going offline. */
                }
            }
        }
    }
